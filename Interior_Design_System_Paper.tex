...
\title{AI-Powered Interior Design System: A Comprehensive Approach to Automated Space Planning and Material Management}
...
\begin{IEEEkeywords}
Interior design, artificial intelligence, web development, material estimation, project management, supplier coordination.
\end{IEEEkeywords}
...
\begin{itemize}
    \item AI-powered design generation from room images using Google's Gemini AI
    \item Automatic material quantity estimation with HSN/SAC codes for the Indian market
    \item Comprehensive execution planning with phase-wise work schedules and labour requirements
    \item Supplier management and bidding system for material procurement
    \item Project tracking and progress monitoring dashboards
    \item Role-based access control for administrators, users, and suppliers
\end{itemize}
...
\begin{itemize}
    \item Reduction in time required for design generation and material estimation by approximately 70\%
    \item Improved accuracy in material quantity calculations, reducing wastage and cost overruns
    \item Enhanced client satisfaction through better visualization and transparent project timelines
    \item Streamlined supplier coordination and bidding processes
    \item Comprehensive project tracking capabilities that enable real-time progress monitoring
    \item A scalable architecture that can accommodate future enhancements and additional features
\end{itemize}
...
\subsection{Overview of AI in Interior Design}
...
\subsection{Related Work in Design Automation}
...
\subsection{Material Estimation Techniques}
...
\subsection{Project Management Systems}
...
\subsection{AI Applications in Construction and Interior Design}
...
\subsection{AI-Powered Design Generation}
...
\subsection{Material Quantity Estimation}
...
\begin{itemize}
    \item Project summary providing an overview of the work scope
    \item Time required breakdown with total duration and phase-specific timelines
    \item Category breakdown showing estimated days per material category
    \item Phase-wise work plan with dependencies between activities
    \item Labour requirements specifying roles, headcount, and duration
    \item Material usage plan mapping materials to specific phases
    \item Site notes and assumptions documenting constraints and considerations
\end{itemize}
...
\subsection{Supplier Management and Bidding System}
...
\subsection{User and Authentication}
...
\subsection{Project and Design Management}
...
\subsection{Material and Execution Plan Models}
...
\subsection{Supplier and Bid Models}
...
\subsection{AI Output Consistency}
...
\begin{itemize}
    \item Detailed prompt engineering with explicit instructions and examples
    \item Output parsing with fallback mechanisms for malformed responses
    \item User feedback loops allowing users to regenerate designs when unsatisfied
    \item Conservative default parameters for AI generation
\end{itemize}

\subsection{Image Storage and Delivery}
...
\begin{itemize}
    \item Straightforward API integration
    \item Automatic optimization and format conversion
    \item Reliable CDN delivery
    \item Reasonable pricing for the expected usage patterns
\end{itemize}
...
The solution involved adding a progress\_logs array to the ExecutionPlan model, where each log entry captures days logged, description, phase, and the user who logged it. The system calculates progress percentages by comparing total days logged against the planned duration.
...
MongoDB's query performance proved adequate for the expected scale, but we implemented indexing on frequently queried fields (user\_id, project\_id, design\_id) to ensure responsive performance as the data grows. The flexible schema also allowed us to add indexes as needed without schema migrations.
...
\begin{itemize}
    \item Frontend form validation using HTML5 validation and custom validators
    \item Backend validation using Flask's request parsing and custom validation logic
    \item Database-level validation through schema constraints where applicable
\end{itemize}

\subsection{File Upload Security}
File uploads are restricted to specific image formats (PNG, JPG, JPEG, WebP) and size limits (16MB maximum). Filenames are sanitized using Werkzeug's secure\_filename function to prevent directory traversal attacks. Cloudinary provides additional security through access controls and automatic malware scanning.

\section{User Experience and Interface Design}

\subsection{Role-Based Interfaces}
...
\begin{itemize}
    \item AI-generated designs, while impressive, may not always align perfectly with user preferences or practical constraints
    \item Material quantities are estimates and require professional verification
    \item The AR visualization module is currently a placeholder and not fully implemented
    \item Real-time collaboration features are absent, limiting multi-user workflows
    \item Payment gateway integration would be needed for complete commercial functionality
\end{itemize}

\subsection{Future Enhancements}
Several areas present opportunities for future development:
\begin{itemize}
    \item Full AR implementation using WebXR for immersive design visualization
    \item 3D model generation from 2D designs for more comprehensive visualization
    \item Real-time collaboration features allowing multiple users to work on projects simultaneously
    \item Mobile application development for on-site access and management
    \item Payment gateway integration for seamless commercial transactions
    \item Advanced analytics and reporting for business insights
    \item Multi-language support to expand the user base
    \item Dark mode implementation for user preference accommodation
    \item Export functionality to various formats (PDF, CAD) for professional integration
\end{itemize}
...
\bibitem{b1} Google, ``Gemini API Documentation,'' 2024. [Online]. Available: https://ai.google.dev/docs
\bibitem{b2} Flask Project, ``Flask Documentation,'' 2024. [Online]. Available: https://flask.palletsprojects.com/
\bibitem{b3} MongoDB Inc., ``MongoDB Documentation,'' 2024. [Online]. Available: https://docs.mongodb.com/
\bibitem{b4} React Documentation, ``React 19 Documentation,'' 2024. [Online]. Available: https://react.dev/
\bibitem{b5} Cloudinary, ``Cloudinary API Documentation,'' 2024. [Online]. Available: https://cloudinary.com/documentation
\end{thebibliography}
